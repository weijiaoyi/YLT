...
class UserCoupon extends \common\components\ARModel
{
    ...
    public static function sendCoupon($uids, $couponId, $number = 1)
    {
        ...
        foreach ($uids as $uid) {
            self::dbInsert('user_coupon', ['coupon_id' => $couponId, 'number' => $number, 'user_id' => $uid,'use_state'=>1, 'valid_time' => date('Y-m-d H:i:s', time() + $coupon->valid_day * 3600 * 24), 'created_at' => self::$time]);
        }
    }
