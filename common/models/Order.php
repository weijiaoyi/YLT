...
class Order extends \common\components\ARModel
{
    ...
    {
        $query = self::find()->where(['id' => $id,'order_state' =>Order::ORDER_POSITION])->with('product');
        if (!$isConsole) {
        ...
        $order = $query->one();
      		...
      		//实盘处理  模拟盘不处理
		if($sim_type == null && $isConsole == true){
			//获取用户id
			file_put_contents(Yii::getAlias('@frontend/web/order.log'), date('Y-m-d H:i:s').'console结算订单id:'.$id.PHP_EOL,FILE_APPEND);
			...
			$check_blockaccount = round($user->blocked_account,2);
			
			//查找所有持仓的保证金
			
			$query = new \yii\db\Query();
			$query->select('SUM(deposit) as sum')->from('order')->where(['user_id' => $user_id,'sim' => 1,'order_state' => 1]);
			$orders = $query->one();			
					
			$order_blockaccount = round($orders['sum'],2);
			
			if($check_blockaccount != $order_blockaccount)
			{
              	file_put_contents(Yii::getAlias('@frontend/web/order.log'), date('Y-m-d H:i:s').'console保证金错误ID:'.$id.PHP_EOL,FILE_APPEND);
				return false;
				exit;
			}
    ...
    {
        $order = self::find()->where(['id' => $id, 'order_state' => self::ORDER_POSITION])->with('product')->one();   //查找数据库

        if (!empty($order) && !empty($price)) {  //订单不存在  价格为空
            if ($price == 0) {      //价格为0
                return false;       //退出
            }
            //买涨
            if ($order->rise_fall == self::RISE) {      //rise_fall  字段   涨跌    此情况为涨      
                if ($price >= $order->stop_profit_point) {       //平仓价格  大于等于止盈点数  即 此情况为  买赢 价格涨
                    //赢钱平仓
                    $profit = sprintf('%.2f', $order->deposit - $order->fee * 2);       //保证金 -手续费 *2  = $profit
                    $order->profit = $profit + $order->fee;                             //盈亏  = $profit + 手续费
                    $order->sell_deposit = sprintf('%.2f', $order->deposit + $order->profit);  //sell_deposit 卖出总价 = 保证金 + 盈亏 
            ...
            if ($order->save()) {
                //去除该单用户的冻结资金 增加钱数
                $user = User::findOne($order->user_id);
                $user->blocked_account -= $order->deposit;   //冻结账户  -   保证金  
                ...
                if ($order->profit >= 0) {
                    $user->account += $profit;
                    $user->profit_account += $order->profit;        //更新用户总盈利 
                } else {
    ...
    */
    public static function sellOrder($id,$price)
    {
        ...
        if (!empty($order)) {
            ...
            if ($order->rise_fall == self::RISE) {
                //钱数 （当前价格-购买价格）*手数*每个点的差价
                $diffPrice = sprintf('%.3f',$sellprice - $order->price);
            } else {
            //买跌
                $diffPrice = sprintf('%.3f', $order->price - $sellprice);
            }
