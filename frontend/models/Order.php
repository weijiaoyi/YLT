...
class Order extends \common\models\Order
{
    ...
    public static function saveOrder($data, $type = 1)
    {
        if ($type == self::TYPE_EY) {
            ...
            if ($user->state == -1) {
                return ['code' => -1, 'info' => '您的账户已冻结，请联系管理员！'];
            }
...
/**
    ...
    public static function saveDianOrder($data,$sim_type=null)
    {
            ...
            if ($data['profit'] > $product->maxrise) {
                return ['code' => -1, 'info' => '不能超过最大止盈点位！'];
            }            
            if (($data['ost']/$product->unit*$product->unit_price)*$data['volHand']>$data['totalMargin']) {
                return ['code' => -1, 'info' => '止损点位超过最大值，请修改后提交！'];
            }
			...
			if($data['bondId']){
			    // 查询优惠券
				    $sql = 'select coupon.amount from user_coupon LEFT JOIN coupon on user_coupon.coupon_id = coupon.id where user_coupon.id = '.$data['bondId'];
     				$boneres = UserCoupon::findBySql($sql)->asArray()->one(); 
              	
              		UserCoupon::updateAll(['use_state' => 2], 'id = '.$data['bondId']); 
             
              		$boneMoney = $boneres['amount'];
              		//var_dump($boneres);
            }
            //exit($boneMoney);
            if($sim_type==null)
            {
                if (($user->blocked_account + $data['totalMargin'] + $data['totalHandlingMoney']) > $user->account + $boneMoney) {
                return ['code' => -1, 'info' => '您的余额已不够支付，请充值！'];
                }
            }
            else
            {
                if (($user->sim_blocked_account + $data['totalMargin'] + $data['totalHandlingMoney']) > $user->sim_account + $boneMoney) {
            ...
            if($data['state']=="1")//买涨
            {
              $order->price = $dataAll->price+$product->unit;//购入价格+基本点位
            ...
            if (!$order->save()) {
                ...
                {
                $user->blocked_account += $order->deposit;
                $user->account = $user->account - $order->fee + $boneMoney;
                $user->save(false);
                //echo 'hehe';
                //exit;
                //经纪人返点
