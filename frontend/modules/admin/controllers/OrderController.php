...
class OrderController extends \admin\components\Controller
{
	...
	//$html = json_encode($query);
        ...
        $html = $query->getTable([
            ...
            'admin.pid' => ['header' => '止盈/止损', 'value' => function ($row) {
                $str = $row['stop_profit_point'].'/'.$row['stop_loss_point'];
                return $str;
    ...
    {
        $query = (new Order)->listQuerysim()->joinWith(['product.dataAll', 'user.parent', 'user.admin'])->manager()->orderBy('order.id DESC');
        $countQuery = (new Order)->listQuerysim()->joinWith(['product.dataAll', 'user.parent', 'user.admin'])->manager();
	...
	//$html = json_encode($query);
        ...
        return $this->render('sim', compact('html', 'profit', 'amount', 'hand', 'fee'));
    }
...
//        $countQuery = (new Order)->listQuery()->joinWith(['product.dataAll', 'user.parent', 'user.admin']);
        ...
        //表头
        $Excel->getActiveSheet()->getStyle('O')->getNumberFormat()->setFormatCode(\PHPExcel_Style_NumberFormat::FORMAT_NUMBER_00);
        $Excel->setActiveSheetIndex(0)->getStyle('A2:O2')->getFont()->setBold(true);
...
//            var_dump($val);die;
            ...
            $Excel->setActiveSheetIndex(0)->setCellValue('C'.$n, $pNickname->getParentLink($val->user->id));
        ...
        //统计
        $Excel->setActiveSheetIndex(0)->mergeCells('A'.$n.':O'.$n);
        ...
        //格式
        $Excel->getActiveSheet()->getStyle('A2:O'.$n)->getBorders()->getAllBorders()->setBorderStyle(\PHPExcel_Style_Border::BORDER_THIN);
        //导出表格
