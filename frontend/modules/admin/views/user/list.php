...
<p class="cl pd-5 mt-20">
    <?php //if ($user_fee == null){echo 123;}else{echo $user_fee;}?>
...
$(function () {
   ...
   $(".list-container").on('click', '.chongzhengBtn', function () {
        var $this = $(this);
        $.prompt('请输入冲正的金额（保证金-冻结金额）', function (value) {
            $.post($this.attr('href'), {money: value}, function (msg) {
                if (msg.state) {
                    $.alert(msg.info || '修改成功');
                } else {
                    $.alert(msg.info);
